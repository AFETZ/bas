...
| **Наземный транспорт — движок на выбор** | `scripts/run_stage_5_ground_vehicle_demo.sh` (`--engine ardupilot\|carla`), `scripts/rover_manual_drive.py`, `scripts/rover_to_issgr_publisher.py`, `scripts/carla_ground_vehicle.py` | Дорожный + вне-дорожный транспорт на выбранном движке: **ArduRover SITL** (ArduPilot-стек) или **CARLA** (пакет 0.9.12 из CAVISE, live/kinematic) | машина реально едет под ручным управлением и двигается в ИССГР рядом с БАС | MANUAL+RC override (ArduRover) / VehicleControl или bicycle (CARLA) → ИССГР `ground_vehicle.{wheeled,offroad}` | ИССГР API, Admin витрина | «дорожный и вне-дорожный транспорт в одном двойнике с БАС, симулятор на выбор» (ТЗ среда моделирования) | 🟢 (ArduRover drive ~120м; CARLA kinematic 62м verified; CARLA live — нужен GPU-сервер; `docs/stage_5_ground_vehicle.md`) |
