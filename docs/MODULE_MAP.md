...
| **ArduRover — наземный транспорт** | `scripts/_rover_sitl_smoke.py`, `scripts/rover_manual_drive.py`, `scripts/rover_to_issgr_publisher.py`, `scripts/run_stage_5_ground_vehicle_demo.sh` | Наземная техника на том же ArduPilot-стеке: дорожный (`rover`/ackermann) + вне дорог (`rover-skid`/skid-steer) | машина реально едет под ручным управлением и двигается в ИССГР рядом с БАС | MANUAL + RC override → MAVLink телеметрия → ИССГР `ground_vehicle.*` | ИССГР API, Admin витрина | «дорожный и вне-дорожный транспорт в одном двойнике с БАС» (ТЗ среда моделирования) | 🟢 (drive verified ~120м, см. `docs/stage_5_ground_vehicle.md`) |
