...
|---|---|---|
| Объектная модель ИССГР | Идентификаторы домена/системы/объекта, классификации, пространственная и временная модели, атрибуты, связи, хранилища | Не учитывается |
| Геопространственные типы | Точка, линия, полигон, растр, покрытие, облако точек, трёхмерное тело, разные СК и трансляция координат | Частично косвенно через 3D-сцену/Sionna, но не как ИССГР-модель |
| Унифицированный классификатор | Классы верхнего уровня: геопространственные объекты, оперативная обстановка, функциональные объекты, растровые/3D отображения и др. | Не учитывается |
| Бортовая часть ИССГР | Бортовая объектно-ориентированная БД, сохранение данных сенсоров, комплексированные данные для управления БАС | Не учитывается |
| Наземная часть ИССГР | Агрегация данных, цифровые двойники группы БАС и пространства, визуализация объектов | Не учитывается |
| Синхронизация БД | Автоматический запуск синхронизации, уровни детализации, пакеты 40/80 байт, multicast при поддержке каналов связи | Не учитывается; здесь моделируются радиоканалы и метрики |
| Имитаторы ведомственных АСУ | REST API, OGC, создание/редактирование ГПИ во внутренней БД | Не учитывается |
...
| AirSim как симулятор графики/сенсоров | Андрончев, Федотенков | ⏳ → ⚠️ 2.2 | **Полный deploy 22.05.2026** на Cosys-AirSim (поддерживаемый UE5.5 форк). Артефакты: `scripts/airsim_{client,stub_server,bridge}.py` + `run_stage_2_2_airsim_overlay.sh` + официальный `cosysairsim==3.3.0` Python pkg + `docs/stage_2_2_airsim_overlay.md`. Wrapper auto-download Linux UE5 packaged build (637 MB) и запускает headless. Verified end-to-end на реальном UE5: `ping=True, server_version=4, scene_objects=209` (Cylinder, Cone, Cube, Quadrotor1_Copter, Propeller_1..4, FrontCenterCamera). Bridge forwards 360 pose-events → `simSetVehiclePose` к реальному UE5 рендеру; image API empty без GPU (WSL2 llvmpipe). Для real-rendering Cosys-AirSim Editor ставится на Windows GPU host через `BAS_AIRSIM_MODE=off BAS_AIRSIM_HOST=<windows-ip>`. Это исполнительная зона Андрончева/Федотенкова, не Физулина. |
