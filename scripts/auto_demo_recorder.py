...
def load_trajectory(path: Path | None) -> list[DemoStep]:
    if path is None:
        return DEFAULT_TRAJECTORY
    data = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(data, list):
        raise SystemExit(f"trajectory JSON must be a list: {path}")
    out: list[DemoStep] = []
    for idx, item in enumerate(data):
        if not isinstance(item, dict):
            raise SystemExit(f"trajectory item {idx} is not an object")
        out.append(DemoStep(
            label=str(item.get("label", f"step_{idx+1}")),
            api=str(item.get("api", "goto")),
            payload=dict(item.get("payload", {})),
            wait_s=float(item.get("wait_s", 2.0)),
            reach_check=bool(item.get("reach_check", False)),
            reach_north=item.get("reach_north"),
            reach_east=item.get("reach_east"),
            reach_alt=item.get("reach_alt"),
            reach_tol_m=float(item.get("reach_tol_m", 4.0)),
            reach_timeout_s=float(item.get("reach_timeout_s", 30.0)),
            screenshot=item.get("screenshot"),
            fpv_screenshot=bool(item.get("fpv_screenshot", False)),
        ))
    return out
...
def main() -> int:
    ...
    ap.add_argument("--trajectory", default=os.environ.get("BAS_AUTO_DEMO_TRAJECTORY", ""),
                    help="Optional JSON trajectory generated by RF pre-scan")
    args = ap.parse_args()
    ...
    if trajectory_path:
        print(f"[recorder] trajectory → {trajectory_path} ({len(trajectory)} steps)", flush=True)
