#!/usr/bin/env python3
"""Minimal Sionna RT CUDA OptiX test — built-in floor scene."""
import sys
import os
os.environ.setdefault("DRJIT_LIBOPTIX_PATH", "/opt/optix-real/libnvoptix.so.1")
os.environ.setdefault("LD_LIBRARY_PATH", "/opt/optix-real:/usr/lib/wsl/lib")
...
# Try the simplest possible scene — Sionna built-in.
try:
    scene = rt.load_scene(rt.scene.floor_wall)
    print(f"loaded floor_wall scene: objects={len(scene.objects)}")
except Exception as e:
    print(f"[floor_wall load failed] {e}")
    sys.exit(1)
...
try:
    ...
    print("OK — Sionna RT live mode works on CUDA OptiX!")
except Exception as e:
