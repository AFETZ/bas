...
Boots admin_web_server в subprocess, hits the UI-critical API endpoints:
  * /api/admin/health → {ok: True}
  * /api/admin/config → runtime wiring flags for UI degraded states
...
def main() -> int:
    ...
    try:
        ...
        # --- 2. /api/admin/config ---
        print("\n[2] /api/admin/config")
        st, body, _ = _get("/api/admin/config")
        assert st == 200
        d = json.loads(body)
        assert d["issgr_url"] == ""
        assert d["has_onboard_db"] is False
        assert d["has_sync_stats"] is False
        print(f"    {d}")
        ...
        # --- 5. /api/admin/tile_grid?n=10&e=10&size=2000 ---
        print("\n[5] /api/admin/tile_grid (10×10, 2km)")
        ...
        # --- 6. /api/admin/tile_grid clamp guard ---
        ...
        # --- 8. /api/admin/onboard_stats — no DB → 404 ---
        print("\n[8] /api/admin/onboard_stats (no DB → 404)")
        ...
        # --- 10. /api/admin/items (no ISSGR → {} fallback) ---
