...
def _scene_object_pose(obj: SceneObject) -> Pose:
    """SceneObject NED → AirSim Pose. Z = -(height/2) ставит центр объекта
    на половину его высоты выше ground (т.е. объект стоит на земле)."""
    return Pose(
        position=Vector3r(x=obj.north_m, y=obj.east_m, z=-obj.height_m / 2.0),
...
def main() -> int:
    ...
    if args.print_stats or (not args.emit_settings and not args.populate
        ...
        s = scene_stats()
        print(f"Urban scene: {s['total_objects']} objects, "
    ...
    if args.emit_settings:
        ...
        if args.settings_output:
            ...
            print(f"settings.json → {args.settings_output} "
                  f"({len(out_str)} bytes, {len(URBAN_SCENE_CATALOG)} _BasUrbanScene objects)")
        else:
            print(out_str)
    ...
    if args.populate:
        ...
        recs = populate_scene(client, spawn_log=args.spawn_log)
        ok = sum(1 for r in recs if r["ok"])
        print(f"[scene] spawned {ok}/{len(recs)} objects "
              f"(prefix=BasUrban_*); log={args.spawn_log or '<none>'}")
